...
import {
  Plus, Trash2, Upload, ChevronDown, ChevronRight, GripVertical, Video, Loader2, ImageIcon, ClipboardList,
} from 'lucide-react';
...
export default function InstructorNewCoursePage() {
  ...
  const removeSection = (tempId: string) => {
    ...
    if (section?.dbId) {
      ...
      api.delete(`/instructor/sections/${section.dbId}`).catch(() => null);
  ...
  const removeLesson = (sectionTempId: string, lessonTempId: string) => {
    ...
    if (lesson?.dbId) {
      ...
      api.delete(`/instructor/lessons/${lesson.dbId}`).catch(() => null);
      setSections((prev) =>
        prev.map((s) =>
          s.tempId === sectionTempId
  ...
  const saveCurriculum = async (publish: boolean) => {
    ...
    try {
      ...
      for (const section of resolvedSections) {
        ...
        if (!sectionDbId) {
          const sectionRes = await api.post(`/instructor/courses/${courseId}/sections`, { title: section.title });
        ...
        for (const lesson of section.lessons) {
          ...
          if (lesson.type === 'quiz') {
            const res = await api.post(
              `/instructor/courses/${courseId}/sections/${sectionDbId}/quizzes`,
