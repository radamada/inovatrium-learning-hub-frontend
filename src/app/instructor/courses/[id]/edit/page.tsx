...
export default function InstructorEditCoursePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: curriculum } = useQuery<Section[]>({
    ...
    enabled: !!id,
    queryFn: () => api.get(`/instructor/courses/${id}/curriculum`).then((r) => r.data),
  });
  ...
  const publishPendingChanges = async () => {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ['instructor-curriculum', id] });
  ...
  const discardPendingChanges = async () => {
    ...
    try {
      ...
      await api.delete(`/instructor/courses/${id}/pending-changes`, { data: { videoIds: pageVideoIds } });
      qc.invalidateQueries({ queryKey: ['instructor-course', id] });
  ...
  const removeLesson = async (sectionTempId: string, lessonTempId: string, dbId?: string) => {
    ...
    if (course?.published && curriculum) {
      ...
      if (sectionsKey(newSections) === publishedKey) {
        ...
        api.delete(`/instructor/courses/${id}/pending-changes`, { data: { videoIds: [] } })
          .then(() => qc.invalidateQueries({ queryKey: ['instructor-course', id] }))
          .catch(() => null);
      }
    }
        ...
        for (const section of resolvedSections) {
          ...
          for (const lesson of section.lessons) {
            if (lesson.type === 'quiz') {
              if (!lesson.dbId) {
                await api.post(
                  `/instructor/courses/${id}/sections/${sectionDbId}/quizzes`,
                  { title: lesson.title, questions: lesson.questions },
                );
              } else {
                await api.patch(
                  `/instructor/courses/${id}/quizzes/${lesson.dbId}`,
                  { title: lesson.title, questions: lesson.questions },
                );
              }
            } else {
