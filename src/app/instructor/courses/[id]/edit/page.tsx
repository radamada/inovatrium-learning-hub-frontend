...
export default function InstructorEditCoursePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Tracks whether we've done the initial curriculum load (to avoid overwriting user edits)
  const [curriculumInitialized, setCurriculumInitialized] = useState(false);

  const { data: course, isLoading: courseLoading } = useQuery({
    queryKey: ['instructor-course', id],
    queryFn: () => api.get(`/instructor/courses/${id}`).then((r) => r.data),
  });
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting }, setValue, watch } = useForm<FormData>({
  ...
  useEffect(() => {
    ...
    // Keep Select components in sync with react-hook-form
  ...
  useEffect(() => {
    if (!course) return; // always wait for course first to avoid race condition

    if (course.pendingChanges?.curriculum) {
      // Pending curriculum exists — always use it (may overwrite a previous live load)
      setSections(
      ...
      // No pending curriculum, use live (only on first load)
      setSections(
        curriculum.map((s) => ({
          dbId: s._id,
          tempId: s._id,
  ...
  const publishPendingChanges = async () => {
    ...
    try {
      await api.patch(`/instructor/courses/${id}/publish-changes`);
      qc.invalidateQueries({ queryKey: ['instructor-course', id] });
  ...
  const discardPendingChanges = async () => {
    ...
    try {
      await api.delete(`/instructor/courses/${id}/pending-changes`);
      qc.invalidateQueries({ queryKey: ['instructor-course', id] });
  ...
  const onSubmitInfo = async (data: FormData) => {
    try {
      await api.patch(`/instructor/courses/${id}`, { ...data, thumbnail: thumbnailUrl || undefined });
      setStep('curriculum');
      qc.invalidateQueries({ queryKey: ['instructor-course', id] });
  ...
  const removeSection = async (tempId: string, dbId?: string) => {
    if (dbId && !course?.published) {
      try {
        await api.delete(`/instructor/sections/${dbId}`);
      } catch {
      ...
      prev.map((s) =>
        s.tempId === sectionTempId
          ? {
              ...
              lessons: [
                ...s.lessons,
                { tempId: crypto.randomUUID(), title: 'Lecție nouă', cdnVideoId: '', duration: 0, isFree: false, uploading: false },
              ],
            }
          : s,
      ),
  ...
  const removeLesson = async (sectionTempId: string, lessonTempId: string, dbId?: string) => {
    if (dbId && !course?.published) {
      try {
        await api.delete(`/instructor/lessons/${dbId}`);
      } catch {
  ...
  const saveCurriculum = async (publish: boolean) => {
    try {
      if (course?.published) {
        ...
        await api.put(`/instructor/courses/${id}/pending-curriculum`, { curriculum: pendingCurriculum });
        qc.invalidateQueries({ queryKey: ['instructor-course', id] });
        toast.success('Curriculum salvat ca modificări în așteptare!');
      } else {
        // Draft course — apply changes directly
        for (const section of sections) {
          let sectionDbId = section.dbId;

          if (!sectionDbId) {
            const res = await api.post(`/instructor/courses/${id}/sections`, { title: section.title });
            sectionDbId = res.data._id;
          } else {
            await api.patch(`/instructor/sections/${sectionDbId}`, { title: section.title });
          }

          for (const lesson of section.lessons) {
            if (!lesson.dbId) {
              await api.post(
                `/instructor/sections/${sectionDbId}/lessons`,
                { title: lesson.title, cdnVideoId: lesson.cdnVideoId, duration: lesson.duration, isFree: lesson.isFree },
                { params: { courseId: id } },
              );
            } else {
              await api.patch(`/instructor/lessons/${lesson.dbId}`, {
                title: lesson.title, cdnVideoId: lesson.cdnVideoId, duration: lesson.duration, isFree: lesson.isFree,
              });
            }
          }
        ...
        if (publish) {
          await api.patch(`/instructor/courses/${id}/publish`);
        }
        ...
        <form onSubmit={handleSubmit(onSubmitInfo)} className="bg-white rounded-xl border p-6 space-y-5 max-w-2xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input type="number" step="0.01" min="0" placeholder="99.99" {...register('price')} className="mt-1" />
